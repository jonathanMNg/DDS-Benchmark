...
def main():
    ...
    select_user_query = """
                            SELECT * FROM benchmark.moviesByRevenue WHERE type='movie' AND revenue> 50000 ORDER BY revenue ASC LIMIT 100 ALLOW  FILTERING;
                        """
    # Output
    rows = session.execute(select_user_query)
    print("Title {0:50} Revenue".format(""))
    print("-"*64)
    for row in (rows):
