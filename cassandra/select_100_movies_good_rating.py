...
def main():
    ...
    select_user_query = """
                            SELECT * FROM benchmark.moviesByRevenue WHERE type='movie' AND vote_average >= 8.0 AND vote_average < 9.0 LIMIT 100 ALLOW  FILTERING;
                        """
    # Output
    rows = session.execute(select_user_query)
    print("Title {0:50} Rating".format(""))
    print("-"*64)
    for row in (rows):
        print("{title:50} {:10.1f}".format(row.vote_average, title=row.title))
